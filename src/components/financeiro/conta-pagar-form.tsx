...
} from "@/app/(dashboard)/financeiro/contas-pagar/actions";
import {
  contaPagarSchema,
  type ContaPagarInput,
  type ContaPagarFormValues,
} from "@/lib/validations/conta-pagar";
...
interface ContaPagarFormProps {
  ...
  fornecedores: Option[];
  contaId?: string;
  defaultValues?: Partial<ContaPagarFormValues>;
}
...
export function ContaPagarForm({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ContaPagarFormValues, unknown, ContaPagarInput>({
    resolver: zodResolver(contaPagarSchema),
    defaultValues: { status: "PENDENTE", ...defaultValues },
  });

  const onSubmit = (values: ContaPagarInput) => {
    setFormError(null);
    startTransition(async () => {
      const result = contaId
        ? await updateContaPagarAction(contaId, values)
        : await createContaPagarAction(values);
